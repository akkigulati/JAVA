...
public class Main {
    ...
    //     int localDia = lefth + righth + 2;

    //     return Math.max(leftDia, Math.max(rightDia, localDia));
    // }
    static class pair {
            int dia;
            int h;
    ...
    public static  pair diameter(Node node){
        ...
        int localDia=left.h+right.h+2;
        int localh=Math.max(left.h,right.h)+1;
        
        pair res=new pair();
        res.h=localh;
        res.dia=Math.max(localDia,Math.max(left.dia,right.dia));
        return res;
